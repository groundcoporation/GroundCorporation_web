...
export default function AdminMemberScreen({ navigation }: any) {
  ...
  const filteredMembers = members.filter(
    (m) =>
      ...
      (m.children &&
        m.children.some((c: any) => c.child_name.includes(searchQuery))) ||
                      ...
                      <View style={styles.parentRow}>
                        ...
                        >
                          <Text style={styles.parentNameText}>
                            {member.name}{" "}
                            {displayChildren[0].isAdult ? "회원" : "학부모"}님
                          </Text>
                          {/* 🚀 관리자 배지 표시 */}
                          {member.role === "admin" && (
                            ...
                            <View style={styles.childNameSection}>
                              ...
                              <View style={styles.miniBadge}>
                                <Text style={styles.miniBadgeText}>
                                  {child.target_class || "반 미지정"}
                                </Text>
                              </View>
                            </View>

                            {/* 수강권 요약 표시 */}
                            {member.user_packages &&
                            member.user_packages.length > 0 ? (
                              <View style={styles.miniPackageInfo}>
                                <Text style={styles.miniPackageName}>
                                  {member.user_packages[0].package_name ||
                                    "수강권"}
                                </Text>
                                <Text style={styles.miniPackageCount}>
                                  {member.user_packages[0].remaining_count}/
                                  {member.user_packages[0].total_count}회
                                </Text>
                              </View>
                            ) : (
                              <Text style={styles.noPackageMini}>
                                수강권 없음
                              </Text>
                            )}
                          </View>
