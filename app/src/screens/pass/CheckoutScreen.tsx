...
export default function CheckoutScreen({ route, navigation }: any) {
  ...
  const handleProceedPayment = () => {
    ...
    if (usedPointsNum > 0 && usedPointsNum < minUsePoint) {
      Alert.alert("알림", `포인트는 ${formatCurrency(minUsePoint)}부터 사용 가능합니다.`);
      return;
    }
    ...
    <SafeAreaView style={styles.safeArea}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.card}>
              {type === "CART" && cartItems.map((item: any, idx: number) => {
                ...
                return (
                  <View key={idx} style={[styles.itemRow, idx !== cartItems.length - 1 && styles.borderBottom]}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.itemName}>{item.pkg.name}</Text>
                      {opt && <Text style={styles.itemOpt}>옵션: {opt.label}</Text>}
                    </View>
                    <View style={{ alignItems: 'flex-end' }}>
                      <Text style={styles.itemPrice}>{formatCurrency(price * item.quantity)}</Text>
          ...
          <View style={styles.section}>
            <View style={styles.pointHeader}>
              ...
              <Text style={styles.myPointText}>보유 {formatCurrency(myPoints)}</Text>
            </View>
            <View style={styles.card}>
              ...
              {myPoints > 0 && minUsePoint > 0 && (
                <Text style={styles.pointNotice}>* 포인트는 {formatCurrency(minUsePoint)}부터 사용 가능합니다.</Text>
              )}
            </View>
          </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.receiptCard}>
              <View style={styles.receiptRow}>
                ...
                <Text style={styles.receiptValue}>{formatCurrency(totalAmount)}</Text>
              </View>
              <View style={styles.receiptRow}>
                <Text style={styles.receiptLabel}>포인트 사용</Text>
                <Text style={[styles.receiptValue, { color: '#EF4444' }]}>
                  {usedPointsNum > 0 ? `-${formatCurrency(usedPointsNum)}` : "0원"}
                </Text>
              </View>
              <View style={styles.divider} />
              <View style={styles.receiptRowTotal}>
                <Text style={styles.receiptLabelTotal}>최종 결제 금액</Text>
                <Text style={styles.receiptValueTotal}>{formatCurrency(finalAmount)}</Text>
              </View>
            </View>
          </View>
      ...
      <View style={[styles.bottomFooter, { paddingBottom: insets.bottom || 20 }]}>
        <TouchableOpacity style={styles.payBtn} onPress={handleProceedPayment}>
          <Text style={styles.payBtnText}>{formatCurrency(finalAmount)} 결제하기</Text>
        </TouchableOpacity>
      </View>
      ...
      {showKSPay && currentUser && (
        <KSPayService
          ...
          paymentData={{
            amount: finalAmount,
            packageName: cartItems.length > 1 
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: "#FFF", borderRadius: 16, padding: 20, borderWidth: 1, borderColor: "#F1F5F9", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.03, shadowRadius: 8, elevation: 2 },
  ...
  itemPrice: { fontSize: 16, fontWeight: "800", color: "#111827", marginBottom: 4 },
  itemQty: { fontSize: 13, color: "#64748B" },
  pointHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12 },
  myPointText: { fontSize: 14, fontWeight: "700", color: "#6366F1" },
  pointInputWrapper: { flexDirection: "row", alignItems: "center", backgroundColor: "#F8FAFC", borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, borderWidth: 1, borderColor: "#E2E8F0" },
  pointInput: { flex: 1, fontSize: 18, fontWeight: "700", color: "#111827", padding: 0 },
  pointUnit: { fontSize: 16, fontWeight: "700", color: "#111827", marginHorizontal: 8 },
  useAllBtn: { backgroundColor: "#EEF2FF", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8 },
  useAllBtnText: { color: "#6366F1", fontSize: 13, fontWeight: "700" },
  pointNotice: { fontSize: 12, color: "#94A3B8", marginTop: 10 },
  receiptCard: { backgroundColor: "#FFF", borderRadius: 16, padding: 20, borderWidth: 1, borderColor: "#F1F5F9" },
  ...
  divider: { height: 1, backgroundColor: "#E2E8F0", marginVertical: 16, borderStyle: "dashed" },
  receiptRowTotal: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  receiptLabelTotal: { fontSize: 16, color: "#111827", fontWeight: "800" },
  receiptValueTotal: { fontSize: 24, color: "#6366F1", fontWeight: "900" },
  bottomFooter: { position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "#FFF", paddingHorizontal: 20, paddingTop: 16, borderTopWidth: 1, borderTopColor: "#F1F5F9" },
