...
export default function ReservationListScreen() {
  ...
  const requestCancel = async (item: any) => {
    ...
    const classDateTime = new Date(
      `${item.class_date}T${item.class_schedules?.start_time}`,
    );
    ...
    if (diffHours < 2) {
      return Alert.alert(
        "취소 불가",
        "수업 시작 2시간 이내에는 앱에서 취소가 불가능합니다. 지점으로 연락주세요.",
      );
    }

    Alert.alert(
      "취소 신청",
      "수업 취소를 신청하시겠습니까? 코치 승인 후 수강권이 복구됩니다.",
      [
        { text: "아니오" },
        {
          text: "신청하기",
  ...
  const renderItem = ({ item }: any) => {
    ...
    const getResStatus = () => {
      if (item.status === "cancel_requested")
        return { label: "취소대기", color: "#F59E0B", bg: "#FFFBEB" };
      if (item.status === "canceled")
        return { label: "취소완료", color: "#EF4444", bg: "#FEF2F2" };
      return { label: "예약확정", color: "#10B981", bg: "#ECFDF5" };
    };

    // 2. 출결 상태 (attendance_status 컬럼)
    const getAttStatus = () => {
      if (!item.attendance_status || item.attendance_status === "확인전")
        return null;
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardHeader}>
          ...
          <View style={[styles.statusBadge, { backgroundColor: resStatus.bg }]}>
            <Text style={[styles.statusLabel, { color: resStatus.color }]}>
              {resStatus.label}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.cardBody}>
          <Text style={styles.classTitle}>
            {item.class_schedules?.target_class}
          </Text>
          <View style={styles.timeRow}>
            <Ionicons name="time-outline" size={16} color="#64748B" />
            <Text style={styles.timeText}>
              {item.class_schedules?.start_time.slice(0, 5)} 시작
            </Text>
          </View>
        </View>
