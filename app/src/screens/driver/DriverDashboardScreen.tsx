...
export default function DriverDashboardScreen({ navigation }: any) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "pickup_settings" },
        () => {
          fetchTodayPickups();
        },
      )
  ...
  const toggleDrivingStatus = () => {
    ...
    Alert.alert(
      ...
      isDriving ? "운행이 종료되었습니다." : "위치 전송이 시작되었습니다.",
    );
  };
  ...
  const handleStudentBoarding = async (
    ...
    const nowKST = dayjs().tz().format("YYYY-MM-DDTHH:mm:ssZ"); // 명시적으로 KST 오프셋 포함
