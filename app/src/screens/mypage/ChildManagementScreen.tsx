...
export default function ChildManagementScreen({ navigation }: any) {
  ...
  const openEditModal = (child: any) => {
    ...
    setChildPhone(
      child.child_phone ? formatPhoneNumber(child.child_phone) : "",
    );
    setModalVisible(true);
  };
  ...
  const handleSave = async () => {
    if (!childName.trim())
      return Alert.alert("알림", "자녀 이름을 입력해주세요.");
    // 💡 생년월일 8자리 검사로 변경!
    if (childBirth.trim().length !== 8)
      return Alert.alert(
        "알림",
        "생년월일 8자리를 입력해주세요. (예: 20150505)",
      );
    if (!userId) return;
    ...
    try {
      ...
      if (isEditing && editingChildId) {
        ...
        const { error } = await supabase
          .from("children")
          .insert({ parent_id: userId, ...payload });
        if (error) throw error;
              ...
              <View key={child.id} style={styles.childCard}>
                <View style={styles.childCardLeft}>
                  <View
                    style={[
                      styles.childAvatar,
                      child.gender === "F" && { backgroundColor: "#FCE7F3" },
                    ]}
                  >
                    <MaterialCommunityIcons
                      name={
                  ...
                  <View>
                    ...
                    <Text style={styles.childBirth}>
                      생년월일: {child.child_birth}
                    </Text>
                    {child.child_phone && (
                      // 💡 DB에서 가져온 순수 번호를 리스트에서 예쁘게 하이픈 처리해서 표시
                      <Text style={styles.childPhoneText}>
                        📱 {formatPhoneNumber(child.child_phone)}
                      </Text>
                    )}
                  </View>
                </View>
                <View style={styles.actionButtons}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => handleDelete(child.id, child.child_name)}
                  >
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            >
              ...
              <View style={styles.inputGroup}>
                <Text style={styles.label}>
                  생년월일 <Text style={{ color: "#EF4444" }}>*</Text>
                </Text>
                <TextInput
                  style={styles.input}
                  value={childBirth}
              ...
              <View style={styles.inputGroup}>
                <Text style={styles.label}>
                  성별 <Text style={{ color: "#EF4444" }}>*</Text>
                </Text>
                <View style={styles.genderRow}>
                  <TouchableOpacity
                    style={[
                      styles.genderBtn,
                      gender === "M" && styles.genderBtnActiveM,
                    ]}
                    onPress={() => setGender("M")}
                  >
                    <Text
                      style={[
                        ...
                        gender === "M" && styles.genderBtnTextActive,
                      ]}
                    >
                      남자
                    </Text>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.genderBtn,
                      gender === "F" && styles.genderBtnActiveF,
                    ]}
                    onPress={() => setGender("F")}
                  >
                    <Text
                      style={[
                        styles.genderBtnText,
                        gender === "F" && styles.genderBtnTextActive,
                      ]}
                    >
                      여자
                    </Text>
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) =>
                    setChildPhone(formatPhoneNumber(text))
                  }
